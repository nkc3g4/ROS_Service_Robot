...
class speeker():
 def __init__(self):
  self.define()
  rospy.Subscriber(self.Topic, String, self.SpeedCB, queue_size=1)
  rospy.Timer(rospy.Duration(self.ResponseSensitivity), self.TimerCB)
  self.WavName=self.Greet
 # self.speek(self.Greet)
  self.speek("您该吃药了")
  rospy.spin()
 ...
 def define(self):
  ...
  if rospy.has_param('~WorkSpaces'):
   ...
   rospy.set_param('~WorkSpaces', 'ros_voice_system')

  if rospy.has_param('~TTS_Topic'):
    pass
  else:
   rospy.set_param('~TTS_Topic', '/voice_tts/tts_topic')

  if rospy.has_param('~Greet_word'):
    pass
  else:
    rospy.set_param('~Greet_word', '您该吃药了')
  ...
  self.path='/home/%s/Music/%s/'%(self.count, self.WorkSpaces)
  self.Topic = rospy.get_param('~TTS_Topic')
  self.Greet = rospy.get_param('~Greet_word')
 ...
 def write_mp3(self,data):
  ...
  if len(self.WavName) > 15:
   FileSubName = self.WavName[:15]
  else:
...
if __name__=="__main__":
 rospy.init_node('baidu_tts_node')
 rospy.loginfo( "baidu_tts initialization system")
 speeker()
