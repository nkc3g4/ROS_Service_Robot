...
class recoder():
 ...
 def __init__(self):
   self.define()
   rospy.Subscriber(self.asr_topic, Int32, self.StartASR, queue_size=1)
   self.pub = rospy.Publisher(self.nlu_topic, String, queue_size=1)
   rospy.Timer(rospy.Duration(self.ResponseSensitivity), self.TimerCB)
   rospy.spin()


 def StartASR(self, data):
  with self.locker:
   if data.data == 1 and (not self.AlreadyASRNow):
     self.AlreadyASRNow = True
     self.recode()
     words = self.reg()
 ...
 def reg(self):
  ...
  HTTP_HEADER=  {  'Content-Type':      'audio/%s;rate=%s'%(self.FORMAT,self.SAMPLING_RATE),
                   'Content-length':    bytes(json.dumps(RegData)) }
 ...
 def define(self):
  ...
  if rospy.has_param('~asr_topic'):
   pass
  else:
   rospy.set_param('~asr_topic', '/voice_system/asr_topic')

  if rospy.has_param('~nlu_topic'):
   pass
  else:
   rospy.set_param('~nlu_topic', '/voice_system/nlu_topic')
...
if __name__=="__main__":
 rospy.init_node('baidu_asr_node')
