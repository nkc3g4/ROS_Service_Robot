...
{
    return sr_init_ex(sr, session_begin_params, aud_src, get_default_input_dev(), notify);
}
...
int sr_start_listening(struct speech_rec *sr)
{
    ...
    {
        sr_dbg("\nsr_start_listening():QISRSessionBegin failed! error code:%d\n", errcode);
        return errcode;
    }
...
int sr_stop_listening(struct speech_rec *sr)
{
    ...
    if (sr->state < SR_STATE_STARTED)
    {
        sr_dbg("Not started or already stopped. state:%d\n",sr->state);
        return 0;
    }
    ...
    while (sr->rec_stat != MSP_REC_STATUS_COMPLETE) 
    {
        ...
        if (MSP_SUCCESS != ret)	
        {
            sr_dbg("\nsr_stop_listening():QISRGetResult failed! error: %d\n", ret);
...
int sr_write_audio_data(struct speech_rec *sr, char *data, unsigned int len)
{
	...
	if (MSP_REC_STATUS_SUCCESS == sr->rec_stat) 
	    ...
	    rslt = QISRGetResult(sr->session_id, &sr->rec_stat, 0, &ret);
            if (MSP_SUCCESS != ret)
            {
	        sr_dbg("\nsr_write_audio_data():QISRGetResult failed! code: %d\n", ret);
	        end_sr_on_error(sr, ret);
	        return ret;
	    }
	    if (NULL != rslt && sr->notif.on_result)
